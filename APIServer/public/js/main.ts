import { APIDoTermResult } from "./_global";
 ...
 * @param {string} elID
 */
export function toggle(elID: string) {
        ...
        getit("my/example/qry/" + curExample._key, {
            ...
            .then((data) => {
                ...
                if (wasGraph) {
                    ...
                    output.textContent += JSON.stringify(data, null, 2).substr(
                        0,
                        400,
                    );
                }
...
function visualise(data) {
    ...
    link.attr("stroke", (d) => getRandCol(d.reduction))
        .attr("stroke-dasharray", (d) => (d.real ? null : "5,5"))
        .attr(
            "marker-end",
            (d) => "url(#" + getRandCol(d.reduction, true) + ")",
        );
