...
(define (amb->kv exp)
  (match exp
    [(list 'amb e ...) (list (cons 'type "ambigous") (cons 'fixed (= 1 (length e))) (cons 'numOptions (length e)))]
...
(define-judgment-form Debug_Amb
   ...
   [
       ; no pause when one possible outcome
       (where (e_0) ,(apply-reduction-relation amb-red (term e)))
     --------------------------------------------------------------------------------------------- "Amb-Resume-One"
       (dstep (debug step e)  (debug step e_0))

   ]
   [   ; Go to pause when multiple options (match at least two elements)
       (where (e_i e_0 ... e_x e_n ...) ,(apply-reduction-relation amb-red (term e)))
     --------------------------------------------------------------------------------------------- "Amb-Resume"
